...
def diffuse(model: ChunkGrid[bool], repeat=1):
    ...
    result = model.copy(dtype=float)

    for r in range(repeat):
        tmp = model.copy(empty=True)
        for chunk in result.chunks:
            conv = signal.convolve(chunk.padding(result, 1), kernel, mode='valid', method='direct')
            conv[model.ensure_chunk_at_index(chunk.index).to_array()] = 1.0
...
dok = dok_matrix((nodes_count + 2, nodes_count + 2), dtype=np.float32)

for vPos, w in tqdm.tqdm(voxels.items(), total=len(voxels), desc="Linking Edges"):
...
for vPos in tqdm.tqdm(list(crust_outer.where()), desc="Linking Source"):
    ...
    for f in ChunkFace:  # type: ChunkFace
        ...
        fIndex = nodes_index[fNode]
        dok[nodes_count, fIndex] = nodes_count
        dok[fIndex, nodes_count] = nodes_count
...
for vPos in tqdm.tqdm(list(crust_inner.where()), desc="Linking Sink"):
    ...
    for f in ChunkFace:  # type: ChunkFace
        fNode = get_node(tuple(vPos), f)
        fIndex = nodes_index[fNode]
        dok[fIndex, nodes_count + 1] = nodes_count
        dok[nodes_count + 1, fIndex] = nodes_count

import scipy.sparse.csgraph as csg

graph = (dok.tocsr() * 10 ** 5).astype(int)
res = csg.maximum_flow(graph, nodes_count, nodes_count + 1)
