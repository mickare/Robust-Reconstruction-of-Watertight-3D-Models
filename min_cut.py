...
for key in sorted_keys:
    ...
    # Add nodes
    set_x = (np.array(key) + (-1, 0, 0)) not in sorted_keys
    set_y = (np.array(key) + (0, -1, 0)) not in sorted_keys
    set_z = (np.array(key) + (0, 0, -1)) not in sorted_keys
    ...
    for i in enumerate(nodes[key])[:-1]:
        for j in enumerate(nodes[key][i])[:-1]:
            for k in enumerate(nodes[key][j])[:-1]:
...
for key, n in nodes:
    ...
    for i in enumerate(n)[:-1]:
        for j in enumerate(n[i])[:-1]:
            for k in enumerate(n[j])[:-1]:
...
complete_surface = []
for key, s in surface:
    offset = key*diffusion.chunk_size
